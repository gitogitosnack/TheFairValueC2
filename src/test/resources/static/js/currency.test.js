...
async function loadModule() {
  vi.resetModules();
  await import("../../../../main/resources/static/js/currency.js");
...
describe("currency.js", () => {
    ...
    document.body.innerHTML = `
      <div id="editModal" class="modal">
        ...
        <form id="editForm">
          ...
          <input id="modalSymbol" name="symbol" />
        </form>
      </div>
      <button id="addCurrencyBtn">追加</button>
      <table>
        <tbody>
          <tr>
            <td>1</td><td>JPY</td><td>¥</td>
  ...
  test("新規登録ボタンを押すとフォームがリセットされモーダルタイトルが変わること", async () => {
    ...
    $("#addCurrencyBtn").trigger("click");

    expect($("#modalId").val()).toBe("");
    expect($("#modalCode").val()).toBe("");
    expect($(".modal-header h2").text()).toBe("通貨の登録");
  });

  test("編集リンクを押すと行のデータがモーダルの入力欄にセットされモーダルタイトルが変わること", async () => {
    await loadModule();

    $(".edit-link").trigger("click");

    expect($("#modalId").val()).toBe("1");
    expect($("#modalCode").val()).toBe("JPY");
    expect($("#modalSymbol").val()).toBe("¥");
    expect($(".modal-header h2").text()).toBe("通貨情報の編集");
  });
  ...
  test("新規登録の送信でinsertエンドポイントにPOSTされ成功時にリロードされること", async () => {
    ...
    expect(options.url).toBe("rest_currencies/insert");
    expect(options.type).toBe("POST");
    const sentData = JSON.parse(options.data);
    expect(sentData.code).toBe("USD");
    expect(sentData.symbol).toBe("$");
  ...
  test("既存IDがあるとき更新エンドポイントにPOSTされること", async () => {
    ...
    const options = ajaxSpy.mock.calls[0][0];
    expect(options.url).toBe("rest_currencies/update");
  ...
  test("削除確認でOKしたときDELETEリクエストが送られること", async () => {
    ...
    const options = ajaxSpy.mock.calls[0][0];
    expect(options.url).toBe("/rest_currencies/delete/1");
