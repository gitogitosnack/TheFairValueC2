...
そのため両市場は単一のロックキー・単一の `batch_execution_logs` 行・単一の cron スケジュール
...
- **処理内容**
  ...
  4. INSERT 成功した銘柄について `AnalysisIndicatorRecalcService` を呼び出し `analysis_indicators`
     を再計算・INSERT（算出ロジック本体は既存の評価モデル機能側、4.3.3 参照）
...
- **実行タイミング**: 4.3 冒頭の通り米国株ステップと同一 `FinancialStatementSyncService` 呼び出し内で
...
- **確定（11 章 回答 6）**: `analysis_indicators`（ROE・PER・自己資本比率等）の**算出ロジック本体は
  ...
  `AnalysisIndicatorRecalcService` は、財務データ更新をトリガに評価モデル機能側のロジックを呼び出し、
...
4.5.3.1・4.5.3.2 は同一の `FinancialStatementSyncService` 呼び出し内で**逐次実行される 1 つの処理の
前半・後半**であり、並行実行される別々のジョブではない（11 章 回答 10）。順序は米国株→日本株を
例として示すが、逆順でもよい。ロックは 4.5.3 開始時に一度取得し、4.5.3.2 が終わるまで解放しない。
...
sequenceDiagram
    ...
    FSS->>DB: companies(US) を取得
    loop 銘柄ごと
        FSS->>DB: financial_statements の最新 (fiscal_year, quarter) を取得
        FSS->>FMP: fetchLatestStatement(code)
        FMP-->>FSS: 財務データ(JSON)
        alt API側が新しい
            FSS->>DB: financial_statements INSERT
            FSS->>Recalc: recalc(companyId)
...
sequenceDiagram
    ...
    EDINET-->>FSS: 書類一覧（secCode, docTypeCode, docID）
    FSS->>FSS: docTypeCode を 有報/半報 に絞り込み
    FSS->>DB: companies(JP) を取得
    FSS->>FSS: secCode(末尾0除去) を companies.code と突合
    loop 一致した銘柄のみ
        FSS->>EDINET: documents/{docID}（type=5, CSV/XBRL代替）
        EDINET-->>FSS: 開示書類データ
        FSS->>FSS: 勘定科目タクソノミをパース
        FSS->>DB: financial_statements INSERT
        FSS->>Recalc: recalc(companyId)
        Recalc->>ValModel: 指標算出ロジック呼び出し
        ValModel-->>Recalc: ROE/PER等の算出結果
...
7. **Step 7**: `FinancialStatementSyncService` の後半（日本株・EDINET）を同一メソッド内に追加し、
   米国株ステップに続けて逐次実行されることを確認する（11 章 回答 10）。XBRL/CSV のタクソノミ
   要素対応はサンプル書類を確認しながら進める（11 章 回答 7）
8. **Step 8**: エラーハンドリング・`market_data_fetch_errors`・レート制限対応を組み込む
...
10. `batch_execution_logs.batch_type` の粒度 → 日米は並行実行ではなく、**同一処理内で逐次実行**
    （順序は任意）。ロックキー・cron・`batch_execution_logs` 行はいずれも日米で分けず単一とする
    （3.4・4.3・6・8.1 参照）
